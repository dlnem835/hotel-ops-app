...
function normalizeInput(body: Record<string, unknown>): PropertyShippingSettingsInput {
  ...
  return {
    ...
    shipFromLine1: String(body.shipFromLine1 ?? body.ship_from_line1 ?? "").trim(),
    shipFromLine2: String(body.shipFromLine2 ?? body.ship_from_line2 ?? "").trim(),
    shipFromCity: String(body.shipFromCity ?? body.ship_from_city ?? "").trim(),
    shipFromState: String(body.shipFromState ?? body.ship_from_state ?? "").trim(),
    shipFromPostal: String(body.shipFromPostal ?? body.ship_from_postal ?? "").trim(),
    shipFromCountry: String(body.shipFromCountry ?? body.ship_from_country ?? "US").trim(),
